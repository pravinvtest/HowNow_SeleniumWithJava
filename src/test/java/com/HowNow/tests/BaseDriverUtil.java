...
import com.HowNow.reusemethods.VideoRecording;
...
public class BaseDriverUtil {
	...
	public synchronized static void ExtentReports() throws IOException {
		prop = new Properties();
		File files = new File(System.getProperty("user.dir") + "/src/test/java/com/HowNow/constants/config.properties");
		...
		try {
			if (extentTest == null) {
				...
				extentSparkReporter = new ExtentSparkReporter(System.getProperty("user.dir") + "./TestReport/IRIS_WebAutomation_ExecutionReport.html");
	...
	public void afterSuite() {
		if (url.equals("https://test-tatshboportal-uae.tatsh.cloud/")) {
			extentReports.setSystemInfo("UAE Test Environment: ", prop.getProperty("UAE_Test_Environment_Url"));
			env = "UAE TEST";
				...
				env = "dev";
				} else if (url.equals("https://test-pulse-it.tatsh.cloud/")) {
				extentReports.setSystemInfo("Italy Test Environment", prop.getProperty("Italy_Test_Environment_Url"));
				env = "Italy-test";
			}
		
		extentSparkReporter.config().setReportName("HowNow Automation Report " );
