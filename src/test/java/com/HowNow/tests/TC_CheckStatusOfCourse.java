...
public class TC_CheckStatusOfCourse extends BaseDriverUtil {
	...
	@Test(priority = 1, enabled = true, description = "Validate status of course ")
	public void ValidatingStatusOfCourse() throws IOException, InterruptedException, AWTException {

		System.out.println("checkstatus of course method started ");
		...
		// String email =
		// LoginData.getJSONObject("TC_Login_001").getString("usernameAssignee");
		...
		String userPerformance = LoginData.getJSONObject("TC_Login_001").getString("UsernameForcheckingLearningTab");
		...
		extentTest.info("User to NavigationBar ");
		dashboardHownow.clickonDashboard();
		extentTest.info("User click on dashboard ");
		dashboardHownow.clickonUserbar();
		extentTest.info("User click on user tab ");
		// dashboardHownow.filterSpecificUserAndClickonUser();
		dashboardHownow.selectuser(userPerformance);
		extentTest.info("After filter , user is selected");
		dashboardHownow.checkStatusofCourse(TC_CreateCourseHownow.nameOfCourse);
		extentTest.info("Course sucessfully completed");
